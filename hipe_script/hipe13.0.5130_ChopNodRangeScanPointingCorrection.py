...
if saveOutput: saveSlicedCopy(slicedFrames, nameBasis+"_slicedFrames_preFF", poolLocation=outputDir)

# Refine the spectral flatfield.
# The data are internally split in a large collection of 'logical sets', that are adjusted to a reference
# The reference is built from all unmasked data by fitting a polynomial
# The shorter the range, the less spectral features are expected, the lower order polynomial you should use.
polyOrder=3
slicedFrames = specFlatFieldRange(slicedFrames,polyOrder=polyOrder, verbose=verbose, excludeLeaks=True, selectedRange=None)
