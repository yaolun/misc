	program ncofrac
c---------------------------------------------------
c calculates spectral line profile for collapsing cloud
c input: ND-time tau, inclination of rotation axis w.r.t. l.o.s.
c       range in ND-distance x, number of theta,phi grid points
c       output file name, input data file 'grid.dta:?'
c output: ND-l.o.s. velocity vz, ND-mass element, position,velocity
c----------------------------------------------------
	parameter(nangle=10,npoints=1000)
	character*20 infile,outfile,txtfile,oldfile
...
c
	...
	write (7,'(a)') infile
	write (7,*)' output data file=:'
	write (7,'(a)') outfile
	write (7,*)' output summary file=:'
	write (7,'(a)') txtfile
	if (test) then
...
c input parameters
	...
	read (*,*,end=999) xrmin,xrmax,delxr
	print *,' number of grid points in theta=?, phi=?'
	read (*,*,end=999) ntmax,npmax
	write(7,*)' nondimensional time tau=:',tau
	write(7,*)' non-dim  xrmin=,xrmax=,delxr=:',xrmin,xrmax,delxr
	write(7,*)' number of grid points in theta=, phi=:',ntmax,npmax
	write(7,*) ' nvmax,vmin,vmax,delv,tau,ninc,(aid(ni),ni=1,ninc)'
	write(7,*) nvmax,vmin,vmax,delv,tau,ninc,(aid(ni),ni=1,ninc)
	write(7,*)' vz,delm,xr,theta,phi,ro,ur,utheta,uphi,dvol'
...
c----------------------------------------------------
c  read 1d data file 'datafile', store in array 'dd'
c----------------------------------------------------
	character*(*) datafile
	logical error
...
c----------------------------------------------------------
c modified from thesis subroutine COLLAPSE
c calculates non-dim-density and velocity of collapse model as a function
c of position at a given non-dimensional time tau
c----------------------------------------------------------
	parameter(nangle=10,npoints=1000)
...
c set so program won't do both zero and 2pi in angle phi
	delph=(phimax-phimin)/float(npmax)
c
c initialize lp
	lp=0
c ang1=theta, ang2=phi
c  step through volume, increment in xr,theta,phi
c if xr .eq. zero, then skip iteration
	nrmin=1
	if(xrmin.eq.0.) nrmin=2
	nskip=0
	do 600 nr=nrmin,nrmax
	  xr= xrmin +float(nr-1)*delxr	
c related to volume element
	  if(nr.eq.nrmax) then
	    delr=abs(xrmax-xr)
	    write(7,*)' last step nr,xr,xrmax,delr',nr,xr,xrmax,delr
	  end if
	  const= xr**2 *delth*delph*delr
c loop over angle theta 
	  do 500 nt= 1,ntmax
	    theta=thetamin +float(nt-1)*delth
	    ang1=theta
	    dvol=abs(sin(theta)*const)
c if theta= pi set dvol=0 to avoid roundoff
	    if(nt.eq.ntmax) dvol=0.
c
c  calculate nearest point in dd array
...
100	    continue
c
c  calculate density,velocity at position xr,theta
c	calculate inner solution
		if (xr.lt.tsq) then
...
c
c calculate mass element, l.o.s. velocity for all inclination angles
	  delm= ro*dvol
c initialize vzold for loop over angle phi
	  phi= phimin -delph
	  do 200 ni=1,ninc
	   call vzpvect(ai(ni),theta,phi,ur,utheta,uphi,vzold(ni))
200	  continue
c loop over angle phi
	  do 400 np= 1,npmax
	        phi= phimin +float(np-1)*delph
	        ang2=phi
	  do 300 ni=1,ninc
c calculate l.o.s. velocity
	    call vzpvect(ai(ni),theta,phi,ur,utheta,uphi,vz(ni))
c check if vz-vzold < delv
...
c calculate velocity channel appropriate to velocity vz
	    nv=nint( (vz(ni)-vmin)/delv +1.0 )
c calculate velocity at left hand side of channel 
	    vl= vmin +(float(nv-1)-0.5)*delv
c calculate velocity at right hand side of channel 
	    vu= vl+delv
	    if (nv.ge. 1 .and. nv.le. npoints) then
c put mass element into velocity profile for inclination ai(ni)
	     if( vl.le.vzold(ni).and.vzold(ni).le.vu ) then
c	      write(7,*)' ni,nv,vz,vzold,v',ni,nv,vz(ni),vzold(ni),v
	      temp= data(nv,ni)
	      data(nv,ni)= data(nv,ni) +delm
c	      write(7,*)' d(nv,ni)new,old',data(nv,ni),temp
	     elseif( vzold(ni).lt.vl) then
	      frac=abs((vz(ni)-vl)/(vz(ni)-vzold(ni)))
c	      write(7,*)' ni,nv,vz,vzold,v,frac',ni,nv,vz(ni),vzold(ni),vl,frac
c	      write(7,*)' d(nv,ni),d(nv-1,ni)old',data(nv,ni),data(nv-1,ni)
	      data(nv,ni)=data(nv,ni) +delm*frac
	      if(nv.gt.1) data(nv-1,ni)=data(nv-1,ni)+delm*(1.0-frac)
c	      write(7,*)' d(nv,ni),d(nv-1,ni)new',data(nv,ni),data(nv-1,ni)
	     else
	      frac=abs((vz(ni)-vu)/(vz(ni)-vzold(ni)))
c	      write(7,*)' ni,nv,vz,vzold,v,frac',ni,nv,vz(ni),vzold(ni),vu,frac
c	      write(7,*)' d(nv,ni),d(nv+1,ni)old',data(nv,ni),data(nv+1,ni)
	      data(nv,ni)=data(nv,ni) +delm*frac
...
c save velocity for checking velocity channel skip
	    vzold(ni)=vz(ni)
c	  write(7,800) vz(ni),delm,xr,theta,phi,ro,ur,utheta,uphi
300	  continue
c if theta=0 then only do phi=0 iteration
	  if(nt.eq.1 .and. thetamin.eq.0.) goto500
c if theta=2pi then only do phi=0 iteration
	  if(nt.eq.ntmax) goto500
c end loop through volume 
...
c-------------------------------------------------------
c calculates physical variables density,velocities
c for collapse solution
c-------------------------------------------------------
...
c-------------------------------------------------
c calculate collapse solution, velocity in cartesian coordinates
c-------------------------------------------------
	tsq=tau*tau
c density-----------------------------------------
45	continue
...
c--------------------------------------------------------------
c calculate Cassen and Moosman inner solution
c--------------------------------------------------------------
	common /collpar/ d2,time,pi,pi2
	data xmo/ .97546863/
...
c
c	print *,' tau,theta,xr',tau,theta,xr
	save
	if(xr.eq.0.) then
		ur=0.
		utheta=0.
...
c
	...
	ao= xmo/(xr)**2
c	print *,' zeta,ao,fv',zeta,ao,fv
c
	call method1(zeta,theta,thetao)
	if( thetao.lt.-1.) then
c		print *,' no thetao: x,zeta,theta=',xr,zeta,theta
c in disk, on equator
		ur=0.
		utheta=0.
		uphi=0.
		ro=0.
...
c	print *,' theta,thetao',theta,thetao
c
c density-----------------------------------------
10	continue
...
c 	ur
50	continue
c	ur= fv*f1(theta,thetao)
c	utheta
c	utheta= f3(theta,thetao)*ur
c
c	uw= ur*sin(theta) +utheta*cos(theta)
c	v=uw
c	return
c Uz----------------------------------------------
c 	ur
...
c	utheta= f3(theta,thetao)*ur
c
c	uz= ur*cos(theta) -utheta*sin(theta)
c	v= uz
c
	return	
	end
...
c----------------------------------------------------------
c  solve transcendental equation 
c  derived by Cassen and Moosman for parabolic trajectories
c  zeta * sin**2(thetao) = 1 - cos(theta)/cos(thetao)
c  given zeta, theta  solve for thetao
c----------------------------------------------------------
...
c
	if (azeta.eq.0.) then
		athetao= atheta
c		type 10, azeta,atheta,athetao
...
c
	...
	xone= 1. 
c
c find roots of cubic equation, put result into array x
...
c		type *,'  x=',x(1),' re(xc)=',x(2),' im(xc)=',x(3)
c		print *,'  x=',x(1)
	else
		costhetao= -.5
		iroot=0
...
c	write (*,5)mtype,zeta,theta,thetao
5	format(' mt=',i5,' zeta=',f8.2,' theta=',f8.2,' thetao=',f8.2)
c
c double precison to single precision
	athetao= thetao
	return
	end	
	subroutine vzpvect(ai,theta,phi,vr,vtheta,vphi,vzp)
c-------------------------------------------
c given spherical velocity vector vr,vtheta,vph 
c spherical coordinates theta,phi and inclination ai (in radians)
c then find the line of sight velocity component vzp
c-------------------------------------------
	sint=sin(theta)
	cost=cos(theta)
	sinp=sin(phi)
	...
	sini=sin(ai)
	cosi=cos(ai)
c	print *,' cos(theta,phi,ai),sin( )',cost,cosp,cosi,sint,sinp,sini
c
	terma=vr*(sint*sinp*sini +cost*cosi)
	termb=vtheta*(cost*sinp*sini -sint*cosi)
	termc=vphi*(cosp*sini)
	vzp=terma +termb +termc
c	print *,' vz,terma,b,c',vz,terma,termb,termc
c
	return 
	end
	subroutine cubesolv(ab,ax,mtype)
c--------------------------------------------------
c solves for roots of cubic equation of form
c   b(1)x**3 +b(2)x**2 +b(3)x +b(4) =0
c mtype = 1   one real, two complex conjugate roots
c       = 0   3 real roots
c       =-1   3 real distinct roots
c solution from CRC Handbook p. A-245
c---------------------------------------------------
...
c	print *,' p,q,r',p,q,r
	...
	det= bb*bb/4. + (ba**3)/27.
c	print *,' ba,bb,det',ba,bb,det
c find solution, check sign of 'det' to find root types
c one real, two complex
	if (det.gt.0.) then
	  ca=(-bb/2. +dsqrt(det))
	  capa=dsign(1.,ca)*(abs(ca))**(1./3.)
	  cb=(-bb/2. -dsqrt(det))
	  capb=dsign(1.,cb)*(abs(cb))**(1./3.)
	  root1= capa + capb
c	  print *,' capa,capb,root1',capa,capb,root1
	  x(1)=root1- p/3.
	  mtype=1
	  goto20
	else if (det.lt.0.) then
c three real distinct roots
	  rg= (-bb/2.)/sqrt(-ba**3/27.)
	  phi= dacos(rg)
	  cons= 2.*dsqrt(-ba/3.)
c	  print *,' rg,phi,cons',rg,phi,cons
...
c
c-------------------------------------------------
c      integrates y on nonuniform grid
c      input independent value array x and dependent
c      value array y of size n
c      xlo and xup are desired bounds of integration
c      ind is an error flag, ind =0 for error
c      x in ascending order; n .ge. 3
c      xlo,xup in interval [x(1),x(n)]
c   
c--------------------------------------------------
c
ccccccccimplicit real*8 (a-h,o-z)
	dimension x(n),y(n)
	ind=0
	if(n.lt.3) return
	do 10 i=2,n
