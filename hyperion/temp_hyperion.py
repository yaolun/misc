def temp_hyperion(rtout,outdir):
    ...
    max_c = np.where(temp == temp.max())
    density = q['density'][0].array.T
    print np.mean(temp[11,:,:])
    print rc[0:15]/AU
    mag = 1.5
    ...
    # cmap = 'jet'
    # im = ax.pcolormesh(thetac_exp, rc/AU, temp2d_exp, cmap=cmap, norm=LogNorm(vmin=temp2d.min(), vmax=temp2d.max()))
