def extract_hyperion(filename, aperture, indir=None, outdir=None, dstar=200.0,
    ...
    if indir == None:
        indir = input('Path to the observation data: ')
    if outdir == None:
        outdir = input('Path for the output: ')
    ...
    if not clean:
        ...
        # print the bolometric luminosity
    ...
    if not clean:
        ...
        # plot the SED of infinite aperture
    ...
    for i in range(0, len(wl_aper)):
        # option of excluding some wavelength channels
    ...
    if len(unc_aper[unc_aper >= flux_aper]) !=0:
        warnings.warn('uncertainties of SED greater than flux found in '+\
                      ".".join(map(str, wl_aper[unc_aper >= flux_aper]))+' microns')

        unc_aper[unc_aper >= flux_aper] = flux_aper[unc_aper >= flux_aper]-1e-20
